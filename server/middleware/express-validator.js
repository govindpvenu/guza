...
const userValidation = [
    ...
    check('phone').isNumeric().withMessage('Mobile number should only contain numbers').isLength({ min: 10 }).withMessage('Mobile number must have atleast 10 numbers').custom(async value => {
    ...
    check('password','Password Must Be at Least 8 Characters').isLength({ min: 8 }).custom(async value => {
...
const addressValidator=[
    ...
    check("state").matches(/^[A-Za-z\s]+$/).withMessage('State must be alphabetic.'),
    check("district").matches(/^[A-Za-z\s]+$/).withMessage('District must be alphabetic.'),
    check("city").matches(/^[A-Za-z\s]+$/).withMessage('City must be alphabetic.'),
