;(function ($) {
    ...
    $(document).on("click", function (event) {
        ...
        if (
            !location.is(event.target) &&
            location.has(event.target).length === 0
        ) {
            $(".box-dropdown-location").fadeOut()
        }
    })
    ...
    $(".left-filter .btn-open-filter, .btn-open-filter-click").on(
        "click",
        function (e) {
            e.preventDefault()
    ...
    //Popup Account
    $(".btn-close-popup-account, .box-account-overlay").on(
        "click",
        function (e) {
    ...
    $(".box-payment-method .list-radio .item-radio input").on(
        "click",
        function () {
            ...
            on: {
                afterInit: function () {
                    ...
                    if ($(".container-slider-padding-" + idx).length > 0) {
                        $(".container-slider-padding-" + idx).css(
                            "padding-left",
                            leftTitle + "px",
                        )
                    }
